...
\begin{figure}[htp]
    \centering
    \includegraphics[width=.4\linewidth]{./figures/diag-corner.png}
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=.6\linewidth]{./figures/diag-arena-axes.png}
...
%
\lstinputlisting[language=JavaScript,caption={Richesta dell'immagine dalla fotocamera},label={lst:init-scene-camera}]{./listings/get-camera-image.js}
%
...
\lstinputlisting[language=JavaScript,caption={Snapshot della scena di supporto per ottenere un immagine elaborabile},label={lst:init-scene-camera}]{./listings/scene-snapshot.js}
%
Una volta ottenuta l'immagine viene processata da \textit{js-aruco2}, ottenendo le posizioni dei marker in coordinate 2D.
\lstinputlisting[language=JavaScript,caption={Ricerca dei marker ArUco},label={lst:init-scene-camera}]{./listings/find-markers.js}
...
\\
Nella \Cref{fig:posit-error} viene mostrata la variazione dell'errore, calcolato durante la stima delle posizioni, di \textit{POSIT}: un errore elevato significa un forte disallineamento tra la posizione reale di un marker e quella stimata nella scena, un errore minore mostrerà risultati più accurati.
%
