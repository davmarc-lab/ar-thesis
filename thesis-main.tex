...
\acrodef{scafi}[ScaFi]{ScaFi (Scala Fields)}
...
\textit{Project Emerge} nasce con l’obiettivo di esplorare il coordinamento di uno sciame di droni riproducendo le dinamiche tipiche di un sistema distribuito reale.
...
\\
Il progetto propone un’implementazione del nucleo logico dell’Aggregate Computing, adottando Scala come linguaggio di programmazione attraverso l’utilizzo del framework \ac{scafi}.
ScaFi è una libreria, scritta in Scala, e framework per per la programmazione aggregata, che implementa una variante della semantica operazionale dell’\ac{hofc}, resa disponibile come \ac{dsl}, fornendo una piattaforma e un insieme di API per la simulazione e l’esecuzione di sistemi e applicazioni aggregate.
...
\begin{itemize}
    ...
    \item rendering in tempo reale degli oggetti virtuali
\end{itemize}
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=.5\linewidth]{./figures/diag-arena-axes.png}
