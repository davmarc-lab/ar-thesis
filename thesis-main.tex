...
Questa l'ibreria analizza i dati di un immagine e attraverso algoritmi e tecniche di Visione Artificiale, ricerca la posizione dei marker ArUco di un dizionario specifico.
...
La classe \texttt{Marker} è stata predisposta per fornire entrambe le possibili \texttt{pose}; nella soluzione implementata verrà utilizzata soltanto la migliore.
%
...
%
La comunicazione con un broker \ac{mqtt} deve avvenire utilizzando una connessione sicura, in quanto il contesto dell'applicazione è sviluppato con il protocollo \textit{HTTPS}; per questo motivo il broker deve poter stabilere connessioni \textit{TLS}.
Soltanto in un ambiente sicuro, il \textit{browser} utilizzato potrà permettere la comunicazione con quest'ultimo.
Per implementare il client \ac{mqtt} viene utilizzata la libreria \textit{mqtt.js} \cite{mqttjs}, permette di configurare e gestire una connessione ad un broker \ac{mqtt}.
\begin{figure}[htp]
    \centering
...
A causa del posizionamento dinamico si ha la necessità di posizionare arbitrariamente la \textit{mesh}. In particolare la base della mesh, contenente le route dei robot, deve essere posizionata sul piano virtuale dell'arena.
Geometricamente vettore normale della base della mesh deve avere stessa direzione ma verso opposto al vettore normale del piano dell'arena.
...
\[
    M_f = M_i \cdot R
\]
In questo caso la matrice $M_i$ rappresenta la matrice di trasformazione della \textit{mesh}, mentre la matrice $M_f$ rappresenta quella dell'arena. Trovando la matrice $R$ è possibile applicare le opportune trasformazioni per orientare una \textit{mesh} nello spazio di coordinate del mondo.
\[
...
\\
Per quanto riguarda l'orientamento dei robot, viene utilizzato l'asse $y$ dell'arena come asse di rotazione, in modo da rispettare quello del simulatore. La rotazione, ogni volta che deve essere aggiornata, viene sommata o sottratta a quella corrente; in caso contrario si perderebbe la rotazione originale utilizzata per posizionare la mesh sull'arena.
...
Ogni volta che viene ricevuto un messaggio, viene normalizzata la posizione ricevuta nelle coordinate dell'arena virtuale: come specificato in precedenza, l'arena simulata e l'arena virutale non solo hanno due sistem di coordinate differenti, ma la dimensione delle due arene è differente.
...
$$
Dove $xSize$ e $ySize$ rappresentano rispettivamente la metà della lunghezza di due lati dell'arena, mentre $simulatedSize$ è la dimensione del mondo dell'arena simulata.
%
